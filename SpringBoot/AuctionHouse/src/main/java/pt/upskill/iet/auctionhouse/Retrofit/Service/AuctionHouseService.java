...
public class AuctionHouseService {
    ...
    public ItemDto getItemById(long id) throws NotFoundException {
        try {
            Response<ItemDto> response = auctionHouseApiInterface.getItemById(id).execute();
            if (response.isSuccessful()) {
                return response.body();
            } else {
                // Lançar exceção específica para item não encontrado
                throw new NotFoundException("Método getItemById -> Item não encontrado com ID: " + id);
            }
        } catch (IOException e) {
    ...
    public void updateItemStatus(long id, StatusDto status) throws NotFoundException {
        ItemDto item = this.getItemById(id); // Busca o item no C#
        if (item.getStatus() == StatusDto.Sold) {
            System.out.println("Item já vendido. Nenhuma ação necessária.");
            return; // Sai sem tentar atualizar
        }

        // Se o item não estiver vendido, tenta atualizar o status
        try {
            int statusCode = status.ordinal();
            Response<Void> response = auctionHouseApiInterface.updateItemStatus(id, statusCode).execute();
